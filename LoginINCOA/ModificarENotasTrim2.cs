...
namespace LoginINCOA
{
    ...
    public partial class ModificarENotasTrim2 : Form
    {
        public int contador;
        ...
        public ModificarENotasTrim2()
        {
            InitializeComponent();

            SqlCommand cmd = new SqlCommand("Select * from Trimestre2", Controlador.Conexiones());
        ...
        private void SeleccionarRegistroAsignatura_Click(object sender, EventArgs e)
        {
            if (DetallesTrim1Sistema.SelectedRows.Count > 0)
            {
                    ...
                    txtPromedio2.Text = DetallesTrim1Sistema.CurrentRow.Cells["promedio2"].Value.ToString();
        ...
        private void txtBuscador_TextChanged(object sender, EventArgs e)
        {
           ...
           DetallesTrim1Sistema.DataSource = integracion.SelectDataTable("SELECT * FROM Trimestre2 WHERE cod_alumno LIKE ('" + txtBuscador.Text + "%')  OR cod_asignatura LIKE ('" + txtBuscador.Text + "%')");
        }

        private void EliminarRegistrosNota_Click(object sender, EventArgs e)
        {
            string query = "DELETE FROM Trimestre2 WHERE cod_alumno=@cod_alumno AND cod_asignatura=@cod_asignatura";
        ...
        private void btnModificarNota_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {         
                        string query = "UPDATE Trimestre2 SET cod_alumno = @cod_alumno, cod_asignatura = @cod_asignatura, Act1 = @Act1, Act2 = @Act2, Exam = @Exam, promedio2 = @promedio2 WHERE cod_alumno=@cod_alumno AND cod_asignatura=@cod_asignatura";
                        ...
                        comando.Parameters.AddWithValue("@promedio2", Convert.ToDecimal(txtPromedio2.Text));
