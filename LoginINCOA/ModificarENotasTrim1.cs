...
namespace LoginINCOA
{
    ...
    public partial class ModificarENotasTrim1 : Form
    {
        public int contador;
        ...
        private void SeleccionarRegistroAsignatura_Click(object sender, EventArgs e)
        {
            if (DetallesTrim1Sistema.SelectedRows.Count > 0)
            {
                    ...
                    txtPromedio2.Text = DetallesTrim1Sistema.CurrentRow.Cells["promedio1"].Value.ToString();
        ...
        private void txtBuscador_TextChanged(object sender, EventArgs e)
        {
           ...
           DetallesTrim1Sistema.DataSource = integracion.SelectDataTable("SELECT * FROM Trimestre1 WHERE cod_alumno LIKE ('" + txtBuscador.Text + "%')  OR cod_asignatura LIKE ('" + txtBuscador.Text + "%')");
        }

        private void EliminarRegistrosNota_Click(object sender, EventArgs e)
        {
            string query = "DELETE FROM Trimestre1 WHERE cod_alumno=@cod_alumno AND cod_asignatura=@cod_asignatura";
        ...
        private void btnModificarNota_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {         
                        string query = "UPDATE Trimestre1 SET cod_alumno = @cod_alumno, cod_asignatura = @cod_asignatura, Act1 = @Act1, Act2 = @Act2, Exam = @Exam, promedio1 = @promedio1 WHERE cod_alumno=@cod_alumno AND cod_asignatura=@cod_asignatura";
